  ...
  for(n = 0 ; n < batchsize_of_data ; n++ )
  {
    for(c = 0 ; c < input_channels ; c++ )
    {
      for( h = padding/2 ; h < input_height+padding/2 ; h++ )
      {
        ...
        {
          h_X[ n*(input_channels*(input_height+padding)*(input_width+padding)) + c*((input_height+padding)*(input_width+padding)) + h*(input_width+padding) + w] = 
                            input[ n*(input_channels*input_height*input_width) + c*(input_height*input_width) + (h-padding/2)*(input_width) + (w-padding/2)];
        }
      }
    }
  }
