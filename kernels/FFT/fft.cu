...
float* pre_processinput(float* input_layer, int pad, int  batch_size, int* il_dim) {
...
float* FFT::forward(int out_size, int channel, int kernel_height, int kernel_width, int pad, int stride, float* kernel, int batch_size, int height, int width, float* input_layer_without_padding)
{
    ...
    float* input_layer_pad_complex = pre_processinput(input_layer_without_padding, pad, batch_size, il_dim);
